...
import { AdminApiService, AdminProjectDetailDto } from '../../../core/services/admin-api.service';
...
@Component({
  ...
  styles: [`
    ...
    .stat-box { background: #f8f9fa; padding: 1rem; border-radius: 4px; border: 1px solid #dee2e6; text-align: center; }
    .stat-box .title { font-size: 0.875rem; color: #6c757d; font-weight: 600; margin-bottom: 0.5rem; }
    .stat-box .val { font-size: 1.5rem; font-weight: 700; color: #0d6efd; }
  `]
})
export class AdminProjectDetailsComponent implements OnInit {
  adminName = '';
  role = '';
  project: AdminProjectDetailDto | null = null;
  isLoading = true;
  error = '';
